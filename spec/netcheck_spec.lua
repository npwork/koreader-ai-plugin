...
describe("netcheck", function()
    describe("targets", function()
        ...
        it("puts /health before a query the endpoint carries", function()
            local targets = NetCheck.targets("https://gateway.example/ai/?token=abc", nil)
            assert.are.equal("https://gateway.example/ai/health?token=abc", targets[1].url)
        end)
    ...
    describe("run and report", function()
        ...
        it("names a slow server when the request outlasts its own handshake", function()
            local check = fake({ dns = 10, tcp = 40, tls = 100, request = 5150 })
            local text = NetCheck.report(check:run({ NetCheck.targets("https://a.example", nil)[1] }, 1))
            assert.is_truthy(text:find("Slowest step: server and transfer to AI, 5000 ms.", 1, true))
        end)
