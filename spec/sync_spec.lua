...
describe("one sync", function()
    ...
    it("is empty when the first step was dismissed, but not in sync", function()
        local outcome = Sync.run({ (step("Library", nil)) })
        assert.is_true(outcome.empty)
        assert.is_false(outcome.in_sync)
    end)

    it("is in sync when no step had anything to say", function()
        local outcome = Sync.run({ (step("Library", false)), (step("Settings", false)) })
        assert.is_true(outcome.in_sync)
        assert.are.equal("", outcome.text)
    end)
end)
